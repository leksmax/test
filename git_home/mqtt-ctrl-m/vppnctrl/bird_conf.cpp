...
{
	char *common_format = (char*)"router id %s; \n\n"
...
void bird_config_ospf(char *buf)
{
	...
	char *intf_format = (char*)"\t\tinterface \"%s\" { \n"
						"\t\t\tstub; \n"
						"\t\t}; \n";
...
void bird_config_ebgp2(char *buf, cJSON *peers)
{
	int i=0;
	int n=0;
	...
	//struct vpn_config_s *dump_config = ctrl_get_tunnel_config();
	//net_tool_get_if_subnet(dump_config->custom_lan_if, local_sub);
	char route_blackholes[1000] = "";
	char route_blackhole[100] = "";
	...
	if (subnets)
	{
		...
		for(i = 0; i < cnt; i++)
		{
			cJSON* item = cJSON_GetArrayItem(subnets, i);
			cJSON* lan_subnet_item = cJSON_GetObjectItem(item, "lan_subnet");
			sprintf(route_blackhole, "\troute %s blackhole; \n", lan_subnet_item->valuestring);
			strcat(route_blackholes, route_blackhole);
		}
	...
	for (i=0; i<n; i++)
	{
		...
		char content[1024] = {0};
		sprintf(content, ebgp_format,
				table_name, table_name, table_name, route_blackholes,
				table_name, table_name, local_item->valuestring, local_as, peer_item->valuestring, peer_as,
				table_name, table_name);
...
void write_bird_conf(cJSON *peers)
{
	...
	if (buf)
	{
		...
		bird_config_ebgp2(buf, peers);
		write_text((char*)"/etc/bird.conf", buf);
		free(buf);
	}
	return;
}
