...
//get the specified ip from subnet
int net_tool_get_ip_from_subnet(char* subnet, int id, char* ipout)
{
	int ret = -1;
	char parse_buf[100];
	strcpy(parse_buf, subnet);
	char* sep = strchr(subnet, '/');
	if (sep)
	{
		int tmp1, tmp2,tmp3,tmp4;
		*sep = 0;
		if (sscanf(parse_buf, "%d.%d.%d.%d", &tmp1, &tmp2, &tmp3, &tmp4) == 4)
		{
			*sep = 0;
			//strcpy(ipout, parse_buf);
			sprintf(ipout, "%d.%d.%d.%d", tmp1, tmp2, tmp3, id);
			ret = 0;
		}
	}
	return ret;
	...
	if (sep)
	{
		...
		*sep = 0;
		if (sscanf(parse_buf, "%d.%d.%d.%d", &tmp1, &tmp2, &tmp3, &tmp4) == 4)
		{
			*sep = 0;
			//strcpy(ipout, parse_buf);
			sprintf(ipout, "%d.%d.%d.1", tmp1, tmp2, tmp3);
			net_tool_num_to_netmask(atoi(sep + 1), maskout);
			ret = 0;
		}
	}
	return ret;
...
int net_tool_get_if_hwaddr2(char *if_name, char *buf)
{
    int ret = -1; 
    struct ifreq ifr;
    struct sockaddr ip_addr;
...
//return us
unsigned long long cal_time_interval(struct timeval *old_time, struct timeval *now_time)
{
	unsigned long long used_time = 0;
	used_time = (now_time->tv_sec - old_time->tv_sec) * 1000000 + now_time->tv_usec - old_time->tv_usec;
    return used_time;
}
...
// Ping函数
unsigned long ping(struct in_addr *dstip, int timeout)
{
	struct timeval *tval;
	int maxfds = 0;
	fd_set readfds;
					...
					if (n > 0 && FD_ISSET(sockfd, &readfds))
					{
						...
						if (n > 1)
						{
							...
							if (from.sin_addr.s_addr == dstip->s_addr)
							{
								...
								{    
									// 正常就退出循环
									used_time = (unsigned long)cal_time_interval(pkt_tv, &now_tv);
									break;
								}
							}
						}
...
char *recv_data(int sockfd, int *len)
{
	...
	if (sockfd > 0)
	{
		while(1)
		{
			recv_ptr = (char*)realloc(recv_ptr, recv_len_total + MAX_ALLOC_LEN);
			if (recv_ptr)
			{
...
cJSON *net_tool_tcp_json_client_with_size(char *host, int port, cJSON *req, char *prefix, int prefix_size)
{
	...
	if (str_req)
	{
		...
		char *send_buf = (char*)malloc(send_len + 4);
...
{
	char *ret = NULL;
	char *web_head = (char*)"POST %s HTTP/1.1\r\n"
		"Host: %s\r\n"
...
char *net_tool_http_client_proxy(char* proxy_host, int proxy_port, char *host, int port, char *uri, char *body, int *recv_len)
{
	char *ret = NULL;
	char *web_head = (char*)"POST %s HTTP/1.1\r\n"
		"Host: %s\r\n"
	...
	char head_buf[1000];
	char uri_buf[200];
	//sprintf(uri_buf, "http://%s:%d%s", host, port, uri);
	sprintf(uri_buf, "http://%s%s", host, uri);
	sprintf(head_buf, web_head, uri_buf, host, strlen(body));
	...
	if (send_buf)
	{
		...
		strcat(send_buf, body);
		send_buf[total_len - 1] = 0;
		ret = net_tool_tcp_client(proxy_host, proxy_port, send_buf, strlen(send_buf), recv_len);
		free(send_buf);
	}
	return ret;
}
...
char* net_tool_https_client(int method, char *host, int port, char *uri, char* str_req, int req_len, char** headers, int headers_cnt, int *recv_len, char* ca_file)
{
	...
	if (method)
	{
		if (ca_file)
		{
			sprintf(cmd_buf, "curl -X POST -d '%s' %s \"https://%s:%d%s\" --cacert %s 2>/dev/null", str_req, headers_buf, host, port, uri, ca_file);
		}
		else
		{
			sprintf(cmd_buf, "curl -X POST -d '%s' %s \"https://%s:%d%s\" -k 2>/dev/null", str_req, headers_buf, host, port, uri);
		}
	}
	else
	{
		if (ca_file)
		{
			sprintf(cmd_buf, "curl -X GET %s \"https://%s:%d%s\" --cacert %s 2>/dev/null", headers_buf, host, port, uri, ca_file);
		}
		else
		{
			sprintf(cmd_buf, "curl -X GET %s \"https://%s:%d%s\" -k 2>/dev/null", headers_buf, host, port, uri);
		}
	}
	...
	if (file)
	{
		...
		while(fgets(buf, sizeof(buf) - 1, file))
		{
			...
			else
			{
				...
				ret = (char*)realloc(ret, old_len + strlen(buf) + 1);
				strcat(ret, buf);
			}
...
cJSON *net_tool_http_json_client_proxy(char *proxy_host, int proxy_port, char *host, int port, char *uri, cJSON *req)
{
	cJSON *ret = NULL;
	char *str_req = cJSON_Print(req);
	if (str_req)
	{
		int recv_len = 0;
		char *response = net_tool_http_client_proxy(proxy_host, proxy_port, host, port, uri, str_req, &recv_len);
		if (response && recv_len > 0)
		{
			/* find \r\n\r\n or \n\n */
			char *ptr = strstr(response, "\r\n\r\n");
...
struct route_info *get_route_list()
{
	...
	if (file)
	{
		...
		{
			struct route_info *one_route = (struct route_info *)malloc(sizeof(struct route_info));
...
{
	char cmd_buf[400];
	sprintf(cmd_buf, "route add -net %s netmask %s dev %s", ip, netmask, dev);
	system(cmd_buf);
}
...
{
	char cmd_buf[400];
	sprintf(cmd_buf, "route del -net %s netmask %s dev %s", ip, netmask, dev);
	system(cmd_buf);
}
...
{
	char cmd_buf[400];
	sprintf(cmd_buf, "route add -net %s gw %s", subnet, gw);
	system(cmd_buf);
}
...
{
	char cmd_buf[400];
	sprintf(cmd_buf, "route del -net %s gw %s", subnet, gw);
	system(cmd_buf);
}
