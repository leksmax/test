...
void *recv_client_data(int sockfd, int *len)
{
    ...
    if (sockfd > 0)
    {
		...
		{
			recv_ptr = realloc(recv_ptr, recv_len_total + need_recv_len);
			if (recv_ptr)
			{
...
void handle_request(ctrl_request_t *request)
{
	...
	if (request->data_size >= 2 && strncmp(request->data, "ts", 2) == 0)
	{
	...
	else if (request->data_size >= 4 && strncmp(request->data, "json", 4) == 0)
	{
...
ctrl_request_t *new_request()
{
	...
	req = malloc(sizeof(ctrl_request_t));
	if (req)
	{
...
ctrl_server_t * ctrl_server_create(char *host, u_short port)
{
	...
	if (listen_fd > 0)
	{
		ret = malloc(sizeof(ctrl_server_t));
		if (ret)
		{
...
{
	int ret = -1;
	struct ctrl_addr_s *addr = malloc(sizeof(struct ctrl_addr_s));
