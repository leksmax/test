...
char* net_tool_https_client(int method, char *host, int port, char *uri, char* str_req, int req_len, char* header, int *recv_len, char* ca_file)
{
	char* ret = NULL;
	char buf[1024];
	char cmd_buf[1024];
	if (method)
	{
		if (ca_file)
		{
			sprintf(cmd_buf, "curl -X POST -d '%s' -H '%s' https://%s:%d%s --cacert %s", str_req, header, host, port, uri, ca_file);
		}
		else
		{
			sprintf(cmd_buf, "curl -X POST -d '%s' -H '%s' https://%s:%d%s -k", str_req, header, host, port, uri);
		}
	}
	else
	{
		if (ca_file)
		{
			sprintf(cmd_buf, "curl -X GET -H '%s' https://%s:%d%s --cacert %s", header, host, port, uri, ca_file);
		}
		else
		{
			sprintf(cmd_buf, "curl -X GET -H '%s' https://%s:%d%s -k", header, host, port, uri);
		}
	}
...
cJSON* net_tool_https_json_client(int method, char *host, int port, char *uri, cJSON* req, char* header, char* ca_file)
{
	cJSON* ret = NULL;
	char *str_req = NULL;
	int recv_len = 0;
	...
	char* str_ret = net_tool_https_client(method, host, port, uri, str_req, str_req?strlen(str_req):0, header, &recv_len, ca_file);
	if (str_ret)
	{
