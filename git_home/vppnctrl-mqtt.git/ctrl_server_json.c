...
cJSON* get_members(char* cloud_host, int cloud_port, char* self_id, char* team_id, int member_type)
{
	...
	cJSON_AddStringToObject(server_item, "ip", "10.1.255.1");
	...
	cJSON_AddStringToObject(server_item, "self", "0");
...
void json_handle_ping_members(cJSON* jsonreq, ctrl_request_t* request)
{
	...
	net_tool_ping_hosts3(members, "ip", "latency", "latency_list", "loss", 5, 10);
...
void json_handle_set_debuglevel(cJSON* jsonreq, ctrl_request_t* request)
{
	...
	if (level_item)
	{
		...
		cJSON_AddStringToObject(res, "ret", "200");
	}
	else
	{
		cJSON_AddStringToObject(res, "ret", "201");
	}
	json_response(res, request);
	cJSON_Delete(res);
}
...
cJSON *json_get_peers(int tunnel_id, char *server)
{
	...
	if (peers_json)
	{
		...
		if (peers_cnt)
		{
			...
			net_tool_ping_hosts2(ret, "peer_vip", "peer_latency", 2);
		}
		cJSON_Delete(peers_json);
	}
	return ret;
...
{
	char *text = process_tool_run_cmd("artmtd -r sn");
	if (text)
	{
		skip_crlf(text);
		strcpy(out, text + 3);
...
{
	char *text = read_text("/module_name");
	if (text)
	{
		skip_crlf(text);
		strcpy(out, text);
...
{
	char *text = read_text("/firmware_version");
	if (text)
	{
		skip_crlf(text);
		strcpy(out, text);
...
{
	char *cmd_out = process_tool_run_cmd("config get http_passwd");
	if (cmd_out)
	{
		skip_crlf(cmd_out);
		if (strlen(cmd_out) <= max_len / 2)
		{
			base64_encode(cmd_out, strlen(cmd_out), out);
		}
		free(cmd_out);
	}
}
...
void get_geoip(char *publicip_buf, char *latitude_buf, char *longtitude_buf)
{
	...
	if (got == 0)
	{
		char *geo_res = net_tool_http_client2(0, "geoip.bigit.com", 80, "/geoip.php", NULL, 0, NULL, &geo_len);
		if (geo_res)
		{
...
void set_syslog_conf(char *log_host, char *log_port, char *log_level, char *vendor_name)
{
	...
	write_text("/tmp/syslog_vppn.conf", buf);
	return;
}
...
cJSON *new_vpn_jsonreq(ctrl_request_t *request)
{
	...
	cJSON *ret = cJSON_Parse(request->data + strlen("json"));
	return ret;
}
