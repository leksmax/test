                 ...
                 ACFG_WLAN_PROFILE_SEC_METH_WPAWPA2))
        {
            ...
            switch (vap_params->security_params.ieee80211w) {
                case ACFG_WLAN_PROFILE_IEEE80211W_OPTIONAL:
                    sprintf(cmd, "%s %d %s %s %s", WPA_SET_NETWORK_CMD_PREFIX, id,
            ...
            sprintf(cmd, "%s %d %s %d", WPA_SET_NETWORK_CMD_PREFIX, id,
...
acfg_set_hapd_config_params(acfg_wlan_profile_vap_params_t *vap_params)
{
             ...
             ACFG_WLAN_PROFILE_SEC_METH_WPAWPA2))
    {
        ...
        switch (vap_params->security_params.ieee80211w) {
            case ACFG_WLAN_PROFILE_IEEE80211W_OPTIONAL:
                sprintf(acfg_hapd_param_list[index], "SET wpa_key_mgmt %s %s", "WPA-PSK", "WPA-PSK-SHA256");
                break;
            case ACFG_WLAN_PROFILE_IEEE80211W_REQUIRED:
                sprintf(acfg_hapd_param_list[index], "SET wpa_key_mgmt %s", "WPA-PSK-SHA256");
                break;
        }
        CHECK_AND_INCREAMENT(index);

        sprintf(acfg_hapd_param_list[index], "SET ieee80211w %d", vap_params->security_params.ieee80211w);
        CHECK_AND_INCREAMENT(index);

        if (vap_params->security_params.ieee80211w > 0) {

            if (vap_params->security_params.assoc_sa_query_max_timeout > 0) {
                sprintf(acfg_hapd_param_list[index], "SET assoc_sa_query_max_timeout %d",
                        vap_params->security_params.assoc_sa_query_max_timeout);
                CHECK_AND_INCREAMENT(index);
            }

            if (vap_params->security_params.assoc_sa_query_retry_timeout > 0) {
                sprintf(acfg_hapd_param_list[index], "SET assoc_sa_query_retry_timeout %d",
            ...
            switch (vap_params->security_params.group_mgmt_cipher) {
                case ACFG_WLAN_PROFILE_GRP_MGMT_CIPHER_AES_128_CMAC:
                    sprintf(acfg_hapd_param_list[index], "SET group_mgmt_cipher %s",
                             "AES-128-CMAC");
                    break;
             ...
             ACFG_WLAN_PROFILE_SEC_METH_WPA2_EAP))
    {
        ...
        switch (vap_params->security_params.ieee80211w) {
            case ACFG_WLAN_PROFILE_IEEE80211W_OPTIONAL:
                sprintf(acfg_hapd_param_list[index], "SET wpa_key_mgmt %s %s", "WPA-EAP", "WPA-EAP-SHA256");
                break;
            case ACFG_WLAN_PROFILE_IEEE80211W_REQUIRED:
                sprintf(acfg_hapd_param_list[index], "SET wpa_key_mgmt %s", "WPA-EAP-SHA256");
                break;
        }
        CHECK_AND_INCREAMENT(index);
