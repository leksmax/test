...
static void wlanifBSteerControlCmnRssiTimeoutHandler(void *cookie) {
    ...
    struct wlanifBSteerControlVapInfo *vap=(struct wlanifBSteerControlVapInfo *)evloopTimeoutCookie2Get(&state->RssiTimeout);
    struct ether_addr * staAddr=(struct ether_addr *)evloopTimeoutCookie3Get(&state->RssiTimeout);

    if(wlanif_getRssi(vap, staAddr,(int *)&event->data.bs_rssi_measurement.rssi) == LBD_NOK)
    {
        dbgf(state->dbgModule, DBGERR,"%s:get Rssi failed\n",__func__);
    }
            ...
            if (state->bandInfo[i].vaps[j].valid) {
                ...
                while(temp)
                {
                    ...
                    if((temp->flag & 2) && (temp->activity == 0))
                    {
                        ath_netlink_bsteering_event_t *event=calloc(1, sizeof(ath_netlink_bsteering_event_t));
                        event->sys_index = if_nametoindex(vap->ifname);
                        event->type = ATH_EVENT_BSTEERING_CLIENT_ACTIVITY_CHANGE;
                        lbCopyMACAddr(temp->macaddr,event->data.bs_activity_change.client_addr);
                        event->data.bs_activity_change.activity=1;
                        temp->activity=1;
                        wlanifBSteerEventsMsgRx(vap->state,event);
                    ...
                    if ((temp->flag & 4) && (temp->activity == 1)) {
                        ...
                        event->type = ATH_EVENT_BSTEERING_CLIENT_ACTIVITY_CHANGE;
                        lbCopyMACAddr(temp->macaddr,event->data.bs_activity_change.client_addr);
                        event->data.bs_activity_change.activity=0;
                        temp->activity=0;
                        wlanifBSteerEventsMsgRx(vap->state,event);
...
wlanifBSteerControlCmnSendRequestRSSI(wlanifBSteerControlHandle_t state,
    ...
    evloopTimeoutCookie3Set(&state->RssiTimeout,(struct ether_addr *) staAddr);
    evloopTimeoutCookie2Set(&state->RssiTimeout, vap);

    evloopTimeoutRegister(&state->RssiTimeout,
...
#define LIST_STATION_CFG_ALLOC_SIZE (3*1024)
    ...
    do {
        ...
        callback(&addr, &bss, isBTMSupported,
                isRRMSupported,
                isMUMIMOSupported,
                isStaticSMPS, OperatingBand , &phyCapInfo, cookie);
    }while(loop_iter != 1);
...
static LBD_STATUS wlanifBSteerControlCmnPerformIoctlWithMAC(
    ...
    if (ioctlReq == IO_OPERATION_ADDMAC){
        snprintf(command,50,"BLACKLIST_ADD %02x:%02x:%02x:%02x:%02x:%02x",staAddr->ether_addr_octet[0],staAddr->ether_addr_octet[1],staAddr->ether_addr_octet[2],staAddr->ether_addr_octet[3],staAddr->ether_addr_octet[4],staAddr->ether_addr_octet[5]);
    }
    else if (ioctlReq == IO_OPERATION_DELMAC){
        snprintf(command,50,"BLACKLIST_DEL %02x:%02x:%02x:%02x:%02x:%02x",staAddr->ether_addr_octet[0],staAddr->ether_addr_octet[1],staAddr->ether_addr_octet[2],staAddr->ether_addr_octet[3],staAddr->ether_addr_octet[4],staAddr->ether_addr_octet[5]);
    }
    else if (ioctlReq == IO_OPERATION_KICKMAC) {
        snprintf(command,50,"DEAUTHENTICATE %02x:%02x:%02x:%02x:%02x:%02x",staAddr->ether_addr_octet[0],staAddr->ether_addr_octet[1],staAddr->ether_addr_octet[2],staAddr->ether_addr_octet[3],staAddr->ether_addr_octet[4],staAddr->ether_addr_octet[5]);
    }
...
static LBD_BOOL wlanifBSteerControlCmnAreAllVAPsReady(
    ...
    for (i = 0; i < wlanif_band_invalid; ++i) {
        ...
        for (j = 0; j < MAX_VAP_PER_BAND; ++j) {
            ...
            if(wlanif_getAcsCacState(state->bandInfo[i].vaps[j].sock,&acsState, &cacState, bssid) !=0)
            {
                dbgf(state->dbgModule, DBGERR,
...
static LBD_STATUS wlanifBSteerControlCmnGetSTAStats(wlanifBSteerControlHandle_t state,
    ...
    wlanif_getStaStats(vap->sock,staAddr,stats);
    return LBD_OK;
}
...
wlanifBSteerControlHandle_t wlanifBSteerControlCreate(struct dbgModule *dbgModule) {
    ...
    do {
        ...
        evloopTimeoutCreate(&state->vapReadyTimeout, "vapReadyTimeout",
                ...
                state);
        evloopTimeoutCreate(&state->RssiTimeout, "RssiTimeout",
                wlanifBSteerControlCmnRssiTimeoutHandler, state);
...
LBD_STATUS wlanifBSteerControlSetCandidateStateForSTA(
    ...
    dbgf(state->dbgModule, DBGERR, "Inside %s:\n",__func__);
    // Set association state
    u_int32_t operation = IO_OPERATION_DELMAC;
    if (!enable) {
        operation = IO_OPERATION_ADDMAC;
