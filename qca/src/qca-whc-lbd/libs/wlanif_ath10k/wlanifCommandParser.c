...
/**
  ...
  */
void wlanif_getNodeAssocEventData(int sock,char *macaddr, int freq,ath_netlink_bsteering_event_t *event)
{
    ...
    char *vht_max_mcs = NULL;
    char *ht_max_mcs = NULL;
...
#define VHT_CAP_MU_BEAMFORMEE_CAPABLE 0x00100000
    ...
    sd = wlanif_sendCommand(sock,command_send,command_leng,command_reply,reply_leng,&recev);
    if(sd >0)
    {
        command_send[command_leng] = '\0';
        dbgf(NULL, DBGERR,"command_send = %s\n", command_send);
    }
    ...
    while (substring) {
        ...
        if (strcmp(tmp_substring_array,macaddr) != 0){
            ...
            while ( parameter_value){
                if (strncmp (parameter_value,"flags",5) == 0) {
                ...
                else if (strncmp (parameter_value,"ex-capability",13) == 0) {
                    char *excapability;
                    ...
                    event->data.bs_node_associated.datarate_info.num_streams = atoi(max_nss);
                }
                else if (strncmp (parameter_value,"max_vhtmcs",10) == 0) {
                    vht_max_mcs = strtok_r(tmp_substring_array,"=",&tmp_substring_array);
                    vhtmaxmcs = atoi(vht_max_mcs);
                }
                else if (strncmp (parameter_value,"max_mcs",7) == 0) {
                    ht_max_mcs = strtok_r(tmp_substring_array,"=",&tmp_substring_array);
                    htmaxmcs = atoi(ht_max_mcs);
                }
...
/**
  ...
  */
void wlanif_getOpModeChangeInd(char *macaddr,ath_netlink_bsteering_event_t *event,int value , LBD_BOOL isbandwidth)
{
    u_int8_t client_addr[IEEE80211_ADDR_LEN];
    wlanif_getMacaddr(macaddr,client_addr);
...
int wlanif_getVapFreq(struct wlanifBSteerControlVapInfo *vap)
{
    ...
    command_leng = strlen(command_send);
    reply_leng = sizeof(command_reply) - 1;
    sd = wlanif_sendCommand(vap->sock,command_send,command_leng,command_reply,reply_leng,&recev);
    if(sd >0)
    {
        command_send[command_leng] = '\0';
        dbgf(NULL, DBGERR,"command_send = %s\n", command_send);
        ...
        while ( parameter_value){
            if (strncmp (parameter_value,"ssid[0]",7) == 0) {
                ...
                *length  = strlen((char*)ssidname);
                dbgf(NULL, DBGERR,"%s ssidname is %s and len is %d \n",__func__,ssidname,*length);
            }
            parameter_value = strtok_r(NULL,",\n",&tmp_substring_array);
        }
        substring = strtok_r(NULL, "\n",&tmp);
...
void wlanif_RssiXingThreshold(struct wlanifBSteerControlVapInfo *vap)
{
    ...
    {
        command_send[command_leng] = '\0';
        dbgf(NULL, DBGERR," %s command_send is %s\n",__func__, command_send);
    }
    command_reply[recev] = '\0';
    dbgf(NULL, DBGERR,"Commaand reply is %s \n",command_reply);
    return;
...
void wlanif_chanutilEventEnable(struct wlanifBSteerControlVapInfo *vap)
{
    ...
    {
        command_send[command_leng] = '\0';
        dbgf(NULL, DBGERR,"COMMAND_SEND in chanutil enable is = %s\n", command_send);
    }
    command_reply[recev] = '\0';
    dbgf(NULL, DBGERR,"Commaand reply is %s \n",command_reply);
    return;
...
int wlanif_getFreq(const char* ifname,const char* radio_ifname)
{
    ...
    command_leng = strlen(command_send);
    reply_leng = sizeof(command_reply) - 1;
    sd = wlanif_sendCommand(sk,command_send,command_leng,command_reply,reply_leng,&recev);
    if(sd >0)
    {
        command_send[command_leng] = '\0';
        dbgf(NULL, DBGERR,"command_send = %s\n", command_send);
...
/**
  ...
  */
int wlanif_getAcsCacState (int sock, int *acs, int *cac, u_int8_t *bssid)
{
    static char command_send[120], command_reply[1028];
    int command_leng,reply_leng,recev, sd;
    char *tmp;
...
#define IEEE80211_CAPINFO_RADIOMEAS 0x1000
    ...
    {
        command_send[command_leng] = '\0';
        dbgf(NULL, DBGERR,"command_send in getDataRateSta  is %s\n", command_send);
    }
    command_reply[recev] = '\0';//steve
    dbgf(NULL,DBGERR," %s recev is %d Command reply is %s \n",__func__,recev,command_reply);
    if((strcmp(command_reply,"") == 0) || (strncmp(command_reply,"FAIL",sizeof("FAIL"))==0))
    {
        *loop_iter = 1;
        return;
    }
        ...
        while ( parameter_value){
            if (m == 0)
            {
                ...
                if (strncmp (parameter_value,"flags",5) == 0) {
                    ...
                    char *max_nss;
                    max_nss = strtok_r(tmp_substring_array,"=",&tmp_substring_array);
                    phyCapInfo->numStreams = atoi(max_nss);
                }
                else if (strncmp (parameter_value,"max_vhtmcs",10) == 0) {
                    vht_max_mcs = strtok_r(tmp_substring_array,"=",&tmp_substring_array);
                    vhtmaxmcs = atoi(vht_max_mcs);
...
void wlanif_getDataRate (struct wlanifBSteerControlVapInfo *vap)
{
    ...
    while (substring) {
        ...
        while ( parameter_value){
            ...
            else if (strncmp (parameter_value,"max_nss",11) == 0) {
                char *max_nss;
                max_nss = strtok_r(tmp_substring_array,"=",&tmp_substring_array);
                vap->phyCapInfo.numStreams = atoi(max_nss);
            }
            else if (strncmp (parameter_value,"vht_max_mcs",11) == 0) {
                vht_max_mcs = strtok_r(tmp_substring_array,"=",&tmp_substring_array);
                vhtmaxmcs = atoi(vht_max_mcs);
            }
            else if (strncmp (parameter_value,"max_mcs",7) == 0) {
...
/**
  ...
  */
void wlanif_getStaStats(int sock, const struct ether_addr *staAddr, struct ieee80211req_sta_stats *stats)
{
    char command[50] = "STA ";
    static char command_send[120], command_reply[1028];
    int command_leng,reply_leng,recev, sd;
    ...
    dbgf(NULL, DBGINFO, "Command is  %s \n", command);
...
void wlanif_getInactStaStats(struct wlanifBSteerControlVapInfo *vap)
{
    ...
    do
    {
        ...
        command_reply[recev] = '\0';
        m=0;
        if ((recev > 0) && (strncmp(command_reply,"FAIL",sizeof("FAIL"))!=0) && (strcmp(command_reply,"") != 0))
        {
                ...
                else
                {
                    if (strcmp(tmp_substring_array,macaddr) != 0){
                        while ( parameter_value){
                            ...
                            {
                                parameter_value=strtok_r(tmp_substring_array,"=",&tmp_substring_array);
                                temp.rssi=(atoi(parameter_value)+95);
                            }
                            parameter_value = strtok_r(NULL,",\n",&tmp_substring_array);
                        }
                    }
            ...
            else
            {
                ...
                while(traverse)
                {
                    if(lbAreEqualMACAddrs(traverse->macaddr,temp.macaddr)) {
                        if((traverse->tx_bytes != temp.tx_bytes) || (traverse->rx_bytes != temp.rx_bytes) || (traverse->tx_rate != temp.tx_rate)) {
                            traverse->flag=flag|1;
                            traverse->tx_bytes = temp.tx_bytes;
                            traverse->rx_bytes = temp.rx_bytes;
                            traverse->tx_rate = temp.tx_rate;
                        }
...
int wlanif_SendBTMRequest(struct wlanifBSteerControlVapInfo *vap,const struct ether_addr *addr, struct ieee80211_bstm_reqinfo_target *reqinfo)
{
    ...
    command_reply[recev] = '\0';
    dbgf(NULL, DBGERR,"command_reply is %s\n", command_reply);
...
int wlanif_SendRRMBcnrptRequest(struct wlanifBSteerControlVapInfo *vap, const struct ether_addr *addr, u_int8_t reqmode, ieee80211_rrm_beaconreq_info_t bcnrpt)
{
    ...
    snprintf(command, 120, "REQ_BEACON %02x:%02x:%02x:%02x:%02x:%02x req_mode=%02d %2x%02x%04x%04x%02x%02x%02x%02x%02x%02x%02x%02x%02x%s%02d%02d%02d%02d%02d%02d%02d",
...
/**
  ...
  */
void wlanif_getBSteerRRMRptEventData(int sock, char *macaddr, const char *msg, ath_netlink_bsteering_event_t *event)
{
