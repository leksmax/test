...
wlanifBSteerControlCmnGetSSID(wlanifBSteerControlHandle_t state, const char *ifname,
    ...
    if(wlanIfLb->ops->getESSID(wlanIfLb->ctx, ifname, (void *)&buf, &length) < 0) {
        dbgf(state->dbgModule, DBGERR, "%s: getESSID failed, ifName: %s",
             __func__, ifname);
...
int wlanIfConfigInit(int isCfg80211)
{
    ...
    if(isCfg80211)
    {
        wlanIfLb = wlanif_config_init(WLANIF_CFG80211);
    }
    else {
        wlanIfLb = wlanif_config_init(WLANIF_WEXT);
    }
...
static LBD_STATUS wlanifBSteerControlCmnPerformMacCmdOnBand(
    ...
    for (i = 0; i < MAX_VAP_PER_BAND; ++i) {
        if (state->bandInfo[band].vaps[i].valid) {
            if(wlanIfLb->ops->setParamMaccmd(wlanIfLb->ctx, state->bandInfo[band].vaps[i].ifname, (void*)&params[0], (sizeof(int) * 2)) < 0) {
                dbgf(state->dbgModule, DBGERR,
...
static LBD_STATUS wlanifBSteerControlCmnPrivIoctlSetParam(
    ...
    dbgf(state->dbgModule, DBGERR, "Inside %s: param %d \n",__func__,paramId);
...
static LBD_STATUS wlanifBSteerControlCmnDumpATFTableOneIface(
    ...
    if(wlanIfLb->ops->getGenericInfoAtf(wlanIfLb->ctx, vap->ifname, IEEE80211_IOCTL_ATF_SHOWATFTBL, &atfInfo, sizeof(struct atftable)) < 0) {
        if (errno == EOPNOTSUPP) {
            dbgf(state->dbgModule, DBGINFO,
...
LBD_STATUS wlanifBSteerControlSetCandidateStateForSTA(
    ...
    for (i = 0; i < candidateCount; i++) {
        ...
        if((wlanIfLb->ops->addDelKickMAC(wlanIfLb->ctx, vap->ifname, operation,(char *) &addr , sizeof(addr))) < 0) {
            dbgf(state->dbgModule, DBGERR,
