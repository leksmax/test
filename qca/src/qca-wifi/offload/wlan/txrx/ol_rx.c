                    ...
                    while (temp_msdu) {
                        ...
                        if (tid < OL_TX_PFLOW_CTRL_MAX_TIDS) {
                            if (nbuf_class.pkt_tid == QDF_TID_VI)
                                PFLOW_CTRL_PDEV_VIDEO_STATS_ADD(pdev, RX_VIDEO_TID_MSDU_STATUS_OK_FROM_FW, 1);

                            if ((tid == QDF_TID_VO && nbuf_class.is_arp == 0 &&
                                        nbuf_class.is_eap == 0 &&
                                        nbuf_class.is_dhcp == 0))
                                PFLOW_TXRX_TIDQ_STATS_ADD(peer, nbuf_class.pkt_tid, RX_MSDU_STATUS_OK_FROM_FW, 1);
