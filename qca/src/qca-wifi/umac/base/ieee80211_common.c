       ...
       for (i = 0, j = 0; (i < ATF_ACTIVED_MAX_CLIENTS)&&(j < ic->atfcfg_set.peer_num_cal); i++)
       {
            if(ic->atfcfg_set.peer_id[i].sta_assoc_status == 1)
            {
                ...
                if (ic->atfcfg_set.peer_id[i].cfg_flag == 1)
                    IEEE80211_ATF_SET_PEER_CFG(wmi_peer_ext_req->atf_peer_ext_info[j].atf_units_reserved);
                else
                    IEEE80211_ATF_RESET_PEER_CFG(wmi_peer_ext_req->atf_peer_ext_info[j].atf_units_reserved);
                ...
                IEEE80211_DPRINTF_IC(ic, IEEE80211_VERBOSE_NORMAL, IEEE80211_MSG_ATF, "Peer mac : %s Groupindex : %d\n",
