...
osif_send_to_nss(os_if_t osif, struct sk_buff *skb, int nwifi)
{
     ...
     */
    if (osifp->iv_txrx_handle->carrier_vow_config) {
        ...
        if (nbuf_class.is_ipv4 &&
                (nss_ipv4_dscp_action_get(nbuf_class.dscp) == NSS_IPV4_DSCP_MAP_ACTION_DONT_ACCEL))
            goto out;
        else if (nbuf_class.is_ipv6 &&
                (nss_ipv6_dscp_action_get(nbuf_class.dscp) == NSS_IPV6_DSCP_MAP_ACTION_DONT_ACCEL))
            goto out;
    }
