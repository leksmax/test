...
static int __ath_thermal_mitigation_attach (struct ath_softc *sc);
static void __ath_thermal_mitigation_detach (struct ath_softc *sc);
...
void Set_TT_timer(struct ath_softc *sc, A_UINT32 ms)
{
...
int16_t get_current_temperature(struct ath_softc *sc)
{
    ...
    retval = ath_hal_get_temp_deg_c(sc->sc_ah, &temp1, &temp2, &temp3, &temp4);
    if (!retval) {
        printk ("%s[%d]: Failed to get the temperature details.\n",__func__,__LINE__);
        return 0;
    }

    if ((txchainmask & 0x1) && (temp1 > temp)) {
        temp = temp1;
    }
    if ((txchainmask & 0x2) && (temp2 > temp)) {
        temp = temp2;
    }
    if ((txchainmask & 0x4) && (temp3 > temp)) {
        temp = temp3;
    }
    if ((txchainmask & 0x8) && (temp4 > temp)) {
        temp = temp4;
    }
...
OS_TIMER_FUNC(thermal_timeout)
{
    ...
    if((sc->sc_tx_on == 0) && (sc->thermal_param.th_cfg.levelconf[sc->thermal_param.th_stats.level].dcoffpercent < 100)) {
        //If traffic was off, then trun it on, we are in the middle of duty cycle
        tt_tx_on(sc);
        //reset the timer to on time %
        Set_TT_timer(sc, sc->thermal_param.th_cfg.dc - (sc->thermal_param.th_cfg.levelconf[sc->thermal_param.th_stats.level].dcoffpercent * sc->thermal_param.th_cfg.dc) /100);
    } else {
        ...
        if(sc->thermal_param.th_cfg.levelconf[sc->thermal_param.th_stats.level].dcoffpercent) {
            ...
            Set_TT_timer(sc, (sc->thermal_param.th_cfg.levelconf[sc->thermal_param.th_stats.level].dcoffpercent * sc->thermal_param.th_cfg.dc + 100 -1)/100);
        } else {
            //reset the timer to level0wait
            Set_TT_timer(sc, LEVEL0WAIT);
        }
...
int ath_config_thermal_mitigation_param(struct ath_softc *sc)
{
    ...
    tt_tx_on(sc);
    sc->thermal_param.th_stats.level = 0;
    if(sc->thermal_param.th_cfg.enable)
    {
        //reset the timer and start it (from level 0)
        Set_TT_timer(sc, LEVEL0WAIT);
    } else {
        //cancel the timer
        Set_TT_timer(sc, 0);
...
int __ath_thermal_mitigation_attach(struct ath_softc *sc)
{
...
void __ath_thermal_mitigation_detach(struct ath_softc *sc)
{
        ...
        struct net_device *dev)
{
    ...
    if (sc->thermal_param.tt_support) {
        ...
        retval = __ath_thermal_mitigation_attach(sc);
        if (retval) {
            sc->thermal_param.tt_support = TH_FALSE;
            sysfs_remove_group(&dev->dev.kobj, &wifi_thermal_group);
            TH_DEBUG_PRINT(TH_DEBUG_LVL0, sc, "%s: unable to initialize TT\n", __func__);
        }
    } else {
        TH_DEBUG_PRINT(TH_DEBUG_LVL0, sc, "%s: TT not supported \n", __func__);
    }
        ...
        struct net_device *dev)
{
    ...
    TH_DEBUG_PRINT(TH_DEBUG_LVL0, sc, "%s: ++\n", __func__);
    if (sc->thermal_param.tt_support) {
        __ath_thermal_mitigation_detach(sc);
        sysfs_remove_group(&dev->dev.kobj, &wifi_thermal_group);
    }
...
ath_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
    ...
    case SIOCGATHSTATS:
        if(((dev->flags & IFF_UP) == 0 ) || ((atomic_read( &ATH_DEV_TO_SC(scn->sc_dev)->sc_nap_vaps_up) <= 0) \
            ...
            printk(" %s[%d]: Commented the return part even if the VAP is not up for debugging.\n",__func__,__LINE__);
            return -ENXIO;
        }
    ...
    case SIOCGATHPHYSTATS:
        if((( dev->flags & IFF_UP) == 0 ) || ((atomic_read( &ATH_DEV_TO_SC(scn->sc_dev)->sc_nap_vaps_up) <= 0) \
                    && (atomic_read( &ATH_DEV_TO_SC(scn->sc_dev)->sc_nsta_vaps_up) <= 0))){
            printk("%s[%d]: Commented the return part even if the VAP is not up.\n",__func__,__LINE__);
            return -ENXIO;
        }
...
#endif
    if (sysfs_create_group(&dev->dev.kobj, &wifi_attr_group)) {
