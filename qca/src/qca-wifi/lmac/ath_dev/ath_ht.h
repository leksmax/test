...
#define ATH_TID_LOCK(_tid)           qdf_spin_lock_bh(&tid->tid_lock);
#define ATH_TID_UNLOCK(_tid)         qdf_spin_unlock_bh(&tid->tid_lock);
#define ATH_TID_LOCK_CREATE(_tid)    qdf_spinlock_create(&tid->tid_lock);
#define ATH_TID_LOCK_DESTROY(_tid)   qdf_spinlock_destroy(&tid->tid_lock);
...
typedef struct ath_atx_tid {
    ...
    qdf_spinlock_t          tid_lock;           /* TID Lock */
} ath_atx_tid_t;
