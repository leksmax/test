...
ar93xx_set_channel(struct ath_hal *ah,  HAL_CHANNEL_INTERNAL *chan)
{
    ...
    printk("b_mode:%d,frac_mode:%d,a_mode_ref_sel:%d,channel_sel:%d,freq:%d,\n",b_mode,frac_mode,a_mode_ref_sel,channel_sel,freq);
    ...
    OS_DELAY(5000);// extra
    //ath_hal_printf(ah, "%s: SYNTH4 = 0x%x\n", __func__, OS_REG_READ(ah, AR_MERLIN_RADIO_SYNTH4));
    ...
    OS_REG_WRITE(ah, AR_MERLIN_RADIO_SYNTH7, reg32);
    OS_DELAY(5000); // extra
    //ath_hal_printf(ah, "%s: SYNTH7 = 0x%x\n", __func__, OS_REG_READ(ah, AR_MERLIN_RADIO_SYNTH7));
