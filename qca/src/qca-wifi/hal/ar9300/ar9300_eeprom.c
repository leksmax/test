...
static ar9300_eeprom_t *jet_default_template = &ar9300_template_generic;

static osprey_eeprom_t *osprey_eeprom_9300[] =
{
    &ar9300_template_osprey,
 ...
 */
static int Osprey_eeprom_template_preference = ar9300_eeprom_template_osprey;
...
bool ar9300_get_temp_deg_c(struct ath_hal *ah, int16_t *temp1, int16_t *temp2, int16_t *temp3, int16_t *temp4)
{
    // OTP address
...
#define ate_temp_code_chain3_7_0_addr 26
    ...
    //@slope_temp=(98,95,97,97); these shld come from ref_file of QDART
    slope_temp[0]=98;
    slope_temp[1]=95;
    slope_temp[2]=97;
    slope_temp[3]=97;
    ...
    OS_REG_RMW_FIELD(ah,AR_PHY_BB_THERM_ADC_4,AR_PHY_BB_THERM_ADC_4_PREFERRED_THERM_CHAIN,0x1);
    latest_therm_value[0]=OS_REG_READ_FIELD(ah,AR_PHY_BB_THERM_ADC_4,AR_PHY_BB_THERM_ADC_4_LATEST_THERM);
    *temp1=(int)(-slope_temp[0]*(latest_therm_value[0]-ate_temp_code[0])/128+ate_temp-40);

    OS_REG_RMW_FIELD(ah,AR_PHY_BB_THERM_ADC_4,AR_PHY_BB_THERM_ADC_4_PREFERRED_THERM_CHAIN,0x2);
    latest_therm_value[1]=OS_REG_READ_FIELD(ah,AR_PHY_BB_THERM_ADC_4,AR_PHY_BB_THERM_ADC_4_LATEST_THERM);
    *temp2=(int)(-slope_temp[1]*(latest_therm_value[1]-ate_temp_code[1])/128+ate_temp-40);

    OS_REG_RMW_FIELD(ah,AR_PHY_BB_THERM_ADC_4,AR_PHY_BB_THERM_ADC_4_PREFERRED_THERM_CHAIN,0x4);
    latest_therm_value[2]=OS_REG_READ_FIELD(ah,AR_PHY_BB_THERM_ADC_4,AR_PHY_BB_THERM_ADC_4_LATEST_THERM);
    *temp3=(int)(-slope_temp[2]*(latest_therm_value[2]-ate_temp_code[2])/128+ate_temp-40);

    OS_REG_RMW_FIELD(ah,AR_PHY_BB_THERM_ADC_4,AR_PHY_BB_THERM_ADC_4_PREFERRED_THERM_CHAIN,0x8);
    latest_therm_value[3]=OS_REG_READ_FIELD(ah,AR_PHY_BB_THERM_ADC_4,AR_PHY_BB_THERM_ADC_4_LATEST_THERM);
    *temp4=(int)(-slope_temp[3]*(latest_therm_value[3]-ate_temp_code[3])/128+ate_temp-40);

    OS_REG_RMW_FIELD(ah,AR_PHY_BB_THERM_ADC_4,AR_PHY_BB_THERM_ADC_4_FORCE_THERM_CHAIN,0x0);
...
bool ar9300_xpa_bias_level_apply(struct ath_hal *ah, bool is_2ghz)
{
     ...
     */
#if !defined(AR9330_EMULATION) && !defined(AR5500_EMULATION)
    int bias;
...
void copy_osprey_modal_eep_header_to_jet(JET_MODAL_EEP_HEADER *jet_modal_header, OSPREY_MODAL_EEP_HEADER *osprey_modal_header)
{
	...
	OS_MEMCPY(jet_modal_header->reserved, 	   osprey_modal_header->reserved, 		sizeof(osprey_modal_header->reserved));
	...
	OS_MEMCPY(jet_modal_header->futureModal, osprey_modal_header->futureModal, 	sizeof(jet_modal_header->futureModal));
	jet_modal_header->xtal_ppm_target    = 0;
	jet_modal_header->xtal_ppm_tolerance = 0;
	jet_modal_header->temp_slope_high_3  = 0;
	jet_modal_header->eep_iqmask 		 = 0;
}
...
void copy_osprey_base_extension_1_to_jet(JET_BASE_EXTENSION_1 *jet_base_ext1, OSPREY_BASE_EXTENSION_1 *osprey_base_ext1)
{
    ...
    OS_MEMCPY(jet_base_ext1->future, osprey_base_ext1->future, sizeof(u_int8_t));
    jet_base_ext1->eep_paprd_trainning_power = 0;
...
void dump_jet_modal_eep_header(JET_MODAL_EEP_HEADER *modal_eep_header)
{
    ...
    printk("\n reserved \n");
    for(i=0; i < MAX_MODAL_RESERVED; i++)
    {
...
bool ar9300_tuning_caps_apply(struct ath_hal *ah)
{
    ...
    if ((eep->base_eep_header.feature_enable & 0x40) >> 6) {
        ...
        if (AR_SREV_HORNET(ah) || AR_SREV_POSEIDON(ah) || AR_SREV_WASP(ah) || AR_SREV_HONEYBEE(ah)) {
            ...
            printk("ar9300_tuning_caps_apply capin=%d capout=%d\n",eep->base_eep_header.params_for_tuning_caps[0], eep->base_eep_header.params_for_tuning_caps[1]);
        } else {
            OS_REG_RMW_FIELD(ah,
