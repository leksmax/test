...
#ifdef ART_BUILD
#ifdef MDK_AP
        tmp_reg2 = FullAddrRead(RST_REVISION_ID_ADDRESS+KSEG);
        printk("ChipRevId=0x%x ah_mac_ver=0x%x\n", tmp_reg2, AH_PRIVATE((ah))->ah_mac_version );
        if (AR_SREV_JET(ah)){
            if ((tmp_reg2 & 0x0fff) != 0x170){
                ecode = HAL_ENOTSUPP;
                goto bad;
            }
        }
#undef RST_REVISION_ID_ADDRESS
        tmp_reg = FullAddrRead(RST_BOOTSTRAP_ADDRESS+KSEG); // always read 0 in JET EMU
#else
