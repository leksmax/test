...
static A_BOOL binCmdStreamParser_Ver1(A_UINT8 *stream, A_UINT32 readStreamLen)
{
    ...
    streamLen = (A_UINT16)(sizeof(_TESTFLOW_CMD_STREAM_HEADER) + cmdStream->cmdStreamHeader.length);
    ...
    opCodeTLV = (A_UINT8)oneCmdHeader->opCode;
    numOfParms = (A_UINT8)oneCmdHeader->numOfParms;
...
static A_BOOL parseBinCmdStreamTLV(A_UINT8 *stream, A_UINT32 readStreamLen)
{
    ...
    if (pCmdStreamHeader->version > CMD_STREAM_VER1) {
        printf("unsupported cmd stream version %d, or mismatch version\n", pCmdStreamHeader->version);
        return(FALSE);
    }
